...
public class PCModGuiOverlay {
    ...
    private static Button btnSelectFavorites = null;
    ...
    private static Field heldPokemonField = null;
    ...
    public static void onGuiInit(GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        btnSelectFavorites = new Button(btnX, btnY + btnGap * 7, btnW, btnH, new StringTextComponent("@favorite \u2605"), b -> toggleTag("@favorite"));
        btnSelectFavorites.visible = false; event.addWidget(btnSelectFavorites);

        btnClearAll = new Button(btnX, btnY + btnGap * 8, btnW, btnH, new StringTextComponent("Clear All"), b -> clearAllSelections());
    ...
    private static void cacheHeldPokemonField() {
        if (heldPokemonField != null) return;
        for (Field f : PokemonScreen.class.getDeclaredFields()) {
            if (f.getType() == Pokemon.class) {
                heldPokemonField = f;
                heldPokemonField.setAccessible(true);
                break;
            }
        }
    }
        ...
        try {
            ...
            if (widget != null) {
                ...
                if (!activeTypes.isEmpty()) {
                    sb.append("@type:").append(String.join(",", activeTypes)).append(" ");
                }
    ...
    public static void onGuiRender(GuiScreenEvent.DrawScreenEvent.Post event) {
        ...
        try {
            if (heldPokemonField != null) {
                heldPokemon = (Pokemon) heldPokemonField.get(screen);
            }
        } catch (Exception e) {}
        ...
        if (box != null) {
            for (int i = 0; i < 30; i++) {
                ...
                if (p != null && p != heldPokemon && FavoriteManager.isFavorite(p.getUUID())) {
                    int col = i % 6;
                    int row = i / 6;
                    ...
                    mc.font.drawShadow(matrixStack, "\u2605", screenX + 22, screenY + 2, 0xFFD700);
                }
            }
        }
        ...
        if (heldPokemon != null && FavoriteManager.isFavorite(heldPokemon.getUUID())) {
            ...
            mc.font.drawShadow(matrixStack, "\u2605", mouseX + 8, mouseY - 8, 0xFFD700);
        }
    ...
    public static void onGuiClose(GuiScreenEvent.InitGuiEvent.Pre event) {
        ...
        btnL100 = null; btnSelectFavorites = null; btnClearAll = null; currentScreen = null; lastSearchOpen = false;
    ...
    private static String checkValuableSelected(List<Pokemon> selected) {
        for (Pokemon p : selected) {
            if (FavoriteManager.isFavorite(p.getUUID())) return "Favorited Pokémon";
    ...
    private static void setFilterButtonsVisible(boolean visible) {
        ...
        if (btnSelectFavorites != null) btnSelectFavorites.visible = visible;
    ...
    private static void clearAllSelections() {
        try {
            ...
            isTypeMenuOpen = false;
            if (btnTypeMain != null) btnTypeMain.visible = true;
